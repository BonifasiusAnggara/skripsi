(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            ...
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('userlevel/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('userlevel/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#peoplemanager").iCheck(d.peoplemanager == 1 ? 'check' : 'uncheck');
    ...
    var t = $('#mainTable table').DataTable({
        ...
        columns: [
            ...
            {data: 'userlevelname'},
            {data: 'bobot'},
            {data: 'peoplemanager', className: "dt-body-center", width: "15%", render: function (data, type, row, meta) {
    ...
    $('#mainTable').selectDTBeOne(t, 'userlevel/delete');
