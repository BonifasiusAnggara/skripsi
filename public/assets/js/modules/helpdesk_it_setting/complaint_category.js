(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#category_code").val() == '') {
                dmafn.errMsg("Please fill Category Code");
                return false;
            }
            if ($("#category").val() == '') {
                dmafn.errMsg("Please fill Category");
                return false;
            }
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('helpdesk_it_setting/Complaint_category/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('helpdesk_it_setting/Complaint_category/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        sDom: 'it<"row"lp>',
        ajax: {
            url: baseUrl + 'helpdesk_it_setting/Complaint_category/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTBeOne(t, 'helpdesk_it_setting/Complaint_category/delete');
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#category_code").val(d.category_code).focus();
