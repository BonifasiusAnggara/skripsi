(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#status_code").val() == '') {
                dmafn.errMsg("Please fill Status Code");
                return false;
            }
            if ($("#complaint_status").val() == '') {
                dmafn.errMsg("Please fill Complaint Status");
                return false;
            }
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('helpdesk_it_setting/Complaint_status/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('helpdesk_it_setting/Complaint_status/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        sDom: 'it<"row"lp>',
        ajax: {
            url: baseUrl + 'helpdesk_it_setting/Complaint_status/getData',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTBeOne(t, 'helpdesk_it_setting/Complaint_status/delete');
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#status_code").val(d.status_code).focus();
        $("#complaint_status").val(d.complaint_status);
